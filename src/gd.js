...
if (proxy_url) {
  ...
  try {
    ProxyAgent = require('proxy-agent')
  } catch (e) { // Not executed npm i proxy-agent
    ProxyAgent = require('https-proxy-agent')
  }
  axins = axios.create({ httpsAgent: new ProxyAgent(proxy_url) })
} else {
...
async function gen_count_body ({ fid, type, update, service_account, limit, tg }) {
  ...
  function render_smy (smy, type, unfinished_number) {
    ...
    if (['html', 'curl', 'tg'].includes(type)) {
      ...
      if (unfinished_number) result += `\nNumber of Folders that are not counted：${unfinished_number}`
      return result
    } else { // Default output json
      return (typeof smy === 'string') ? smy : JSON.stringify(smy)
    }
  ...
  const record = db.prepare('SELECT * FROM gd WHERE fid = ?').get(fid)
  if (!file && !record) {
    throw new Error(`Unable to access the link，Please check if the link is valid and the SA has the appropriate permissions：https://drive.google.com/drive/folders/${fid}`)
  }
  if (!record || update) {
    [info, smy] = await update_info()
  }
...
function get_all_by_fid (fid) {
  ...
  function recur (result, subf) {
    ...
    const arr = subf.map(v => {
      const row = db.prepare('SELECT * FROM gd WHERE fid = ?').get(v)
      if (!row) return null // If the corresponding fid record is not found, it means that the process was interrupted last time or the directory was not read completely
      let info = JSON.parse(row.info)
      info = info.map(vv => {
        vv.parent = v
        return vv
...
async function walk_and_save ({ fid, not_teamdrive, update, service_account, with_modifiedTime, tg }) {
  ...
  const loop = setInterval(() => {
    const now = dayjs().format('HH:mm:ss')
    const message = `${now} | Number of Files copied ${result.length} | Ongoing 中${limit.activeCount} | Pending ${limit.pendingCount}`
    print_progress(message)
  }, 1000)
  ...
  console.log('\nInfo Obtained')
  unfinished_folders.length ? console.log('Unread Folder ID：', JSON.stringify(unfinished_folders)) : console.log('All Folders have been read')
  clearInterval(loop)
  if (tg_loop) {
...
function save_files_to_db (fid, files) {
  // Do not save the unfinished folder of the request, then the next call to get_all_by_id will return null, thus calling walk_and_save again to try to complete the request for this Folder
...
async function ls_folder ({ fid, not_teamdrive, service_account, with_modifiedTime }) {
  ...
  params.pageSize = Math.min(PAGE_SIZE, 1000)
  // const use_sa = (fid !== 'root') && (service_account || !not_teamdrive) // Without parameters, SA is used by default
  const use_sa = (fid !== 'root') && service_account
  // const headers = await gen_headers(use_sa)
  // For Folder s with a large number of Subfolders (1ctMwpIaBg8S1lrZDxdynLXJpMsm5guAl), the access_token may expire before the listing is complete
  // Since nextPageToken is needed to get the data of the next page, parallel requests cannot be used. The test found that each request for 1000 files takes more than 20 seconds to complete.
  const gtoken = use_sa && (await get_sa_token()).gtoken
  do {
    if (pageToken) params.pageToken = pageToken
    let url = 'https://www.googleapis.com/drive/v3/files'
    ...
    if (!data) {
      console.error('Partially read Folder, Pamters:', params)
...
async function get_sa_token () {
  ...
  while (SA_TOKENS.length) {
    ...
    try {
      ...
      console.warn('SA failed to obtain access_token：', e.message)
      SA_TOKENS = SA_TOKENS.filter(v => v.gtoken !== tk.gtoken)
      if (!SA_TOKENS.length) SA_TOKENS = get_sa_batch()
    }
  }
...
async function real_get_sa_token (el) {
  const { value, expires, gtoken } = el
  // The reason for passing gtoken out is that when an account's daily limit is exhausted, it can be filtered accordingly
  if (Date.now() < expires) return { access_token: value, gtoken }
  const { access_token, expires_in } = await gtoken.getToken({ forceRefresh: true })
  el.value = access_token
  el.expires = Date.now() + 1000 * (expires_in - 60 * 5) // 5 minutes is fixed as expiry in advance
  return { access_token, gtoken }
}
...
async function create_folder (name, parent, use_sa, limit) {
  ...
  while (retry < RETRY_LIMIT) {
    try {
      ...
      console.log('Creating Folder and Retrying：', name, 'Number of retries：', retry)
    }
  }
  throw new Error(err_message + ' Folder Name：' + name)
}
...
async function get_info_by_id (fid, use_sa) {
  ...
  // throw new Error('Unable to Access this link：' + fid)
}
...
async function user_choose () {
  const answer = await prompts({
    type: 'select',
    name: 'value',
    message: 'Do you wish to resume your transfers ？',
    choices: [
      { title: 'Continue', description: 'Continue from where you left off', value: 'continue' },
      { title: 'Restart', description: 'Copy everything once again', value: 'restart' },
      { title: 'Exit', description: 'Exit', value: 'exit' }
    ],
    initial: 0
  })
...
async function copy ({ source, target, name, min_size, update, not_teamdrive, service_account, dncnr, is_server }) {
  ...
  if (!file) return console.error(`Unable to access the link，Please check if the link is valid and the SA has the appropriate permissions：https://drive.google.com/drive/folders/${source}`)
  if (file && file.mimeType !== FOLDER_TYPE) {
    return copy_file(source, target, service_account).catch(console.error)
  }

  const record = db.prepare('select id, status from task where source=? and target=?').get(source, target)
  if (record && record.status === 'copying') return console.log('This Task is already running, Force Exit')
...
// To be resolved: If the user manually ctrl+c interrupts the process, the request that has been sent will not be recorded in the local database even if it is completed, so duplicate files (folders) may be generated
async function real_copy ({ source, target, name, min_size, update, dncnr, not_teamdrive, service_account, is_server }) {
  async function get_new_root () {
    if (dncnr) return { id: target }
    if (name) {
      ...
      const file = await get_info_by_id(source, service_account)
      if (!file) throw new Error(`Unable to access the link，Please check if the link is valid and the SA has appropriate permissions：https://drive.google.com/drive/folders/${source}`)
...
async function copy_files ({ files, mapping, service_account, root, task_id }) {
  ...
  const loop = setInterval(() => {
    const now = dayjs().format('HH:mm:ss')
    const message = `${now} | Number of files copied ${count} | Ongoing ${concurrency} | Pending ${files.length}`
    print_progress(message)
  }, 1000)
...
async function copy_file (id, parent, use_sa, limit, task_id) {
  ...
  while (retry < RETRY_LIMIT) {
    ...
    try {
      ...
      if (!use_sa && message && message.toLowerCase().includes('rate limit')) {
        throw new Error('Daily usage limit exceeded：' + message)
      }
      if (use_sa && message && message.toLowerCase().includes('rate limit')) {
        retry--
        if (gtoken.exceed_count >= EXCEED_LIMIT) {
          SA_TOKENS = SA_TOKENS.filter(v => v.gtoken !== gtoken)
          if (!SA_TOKENS.length) SA_TOKENS = get_sa_batch()
          console.log(`This account reached its ${EXCEED_LIMIT} daily limit continuously，The remaining available SAs are：`, SA_TOKENS.length)
        } else {
            // console.log('This account has exceeded its 750GB quota，its marked，if the next request is normal it will be unmarked，or else the SA will be removed')
          if (gtoken.exceed_count) {
            gtoken.exceed_count++
          } else {
            gtoken.exceed_count = 1
  ...
  if (use_sa && !SA_TOKENS.length) {
    ...
    console.warn('Failed to copy the File，Fileid: ' + id)
  }
}
...
async function create_folders ({ source, old_mapping, folders, root, task_id, service_account }) {
  ...
  const missed_folders = folders.filter(v => !mapping[v.id])
  console.log('Started Copying Folders，Total：', missed_folders.length)
  const limit = pLimit(PARALLEL_LIMIT)
  let count = 0
  let same_levels = folders.filter(v => v.parent === folders[0].parent)
  ...
  while (same_levels.length) {
    ...
    await Promise.all(same_levels_missed.map(async v => {
      try {
        ...
        console.error('Eoor Creating Folder:', e.message)
      }
    }))
    // folders = folders.filter(v => !mapping[v.id])
    same_levels = [].concat(...same_levels.map(v => folders.filter(vv => vv.parent === v.id)))
...
function find_dupe (arr) {
  ...
  for (const file of files) {
    const { md5Checksum, parent, name } = file
    // According to the file location and md5 value to determine whether it is duplicated
    const key = parent + '|' + md5Checksum // + '|' + name
    if (exists[key]) {
      dupe_files.push(file)
    } else {
...
async function confirm_dedupe ({ file_number, folder_number }) {
  const answer = await prompts({
    type: 'select',
    name: 'value',
    message: `Duplicate Files ${file_number}，Empty Folders ${folder_number} are found，Do you want to delete them ？`,
    choices: [
      { title: 'Yes', description: 'Confirm', value: 'yes' },
      { title: 'No', description: 'Dont Delete', value: 'no' }
    ],
    initial: 0
  })
  return answer.value
...
// Sa Should be the manager where source Folder is located
async function mv_file ({ fid, new_parent, service_account }) {
  const file = await get_info_by_id(fid, service_account)
  if (!file) return
  const removeParents = file.parents[0]
...
// Move files or folders to the recycle bin, SA should be Content manager or above
async function trash_file ({ fid, service_account }) {
  const url = `https://www.googleapis.com/drive/v3/files/${fid}?supportsAllDrives=true`
  const headers = await gen_headers(service_account)
  return axins.patch(url, { trashed: true }, { headers })
...
// Delete files or folders directly without moving to recycle bin, and SA should have the Manager Permission
async function rm_file ({ fid, service_account }) {
  const headers = await gen_headers(service_account)
  let retry = 0
  const url = `https://www.googleapis.com/drive/v3/files/${fid}?supportsAllDrives=true`
...
async function dedupe ({ fid, update, service_account, yes }) {
  ...
  await Promise.all(dupes.map(async v => {
    try {
      await limit(() => trash_file({ fid: v.id, service_account }))
      if (v.mimeType === FOLDER_TYPE) {
        console.log('Folder deleted successfully', v.name)
        folder_count++
      } else {
        console.log('File deleted Successfully', v.name)
        file_count++
      }
    } catch (e) {
      console.log('Failed to Delete', v)
